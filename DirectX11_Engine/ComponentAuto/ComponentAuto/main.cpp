...
int main(int Argc, char** Argv)
{
    ...
    {
        const bool bOk = InsertLinesAfterMarkerLine(GameObjectText, "// GET COMPONENT", GameObjectMacroLinesToInsert);
        if (!bOk)
        {
            std::cout << "ERROR: Marker not found in GameObject.h: \"// GET COMPONENT\"\n";
            std::string Dummy;
            std::getline(std::cin, Dummy);
            return 1;
        }
