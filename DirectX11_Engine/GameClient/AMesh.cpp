...
// 부모인 Asset에 디폴트 생성자가 없어서
// Asset(ASSET_TYPE) 생성자를 호출해야함
AMesh::AMesh()
...
int AMesh::Create(Vtx* _VtxSysMem, UINT _VtxCount, UINT* _IdxSysMem, UINT _IdxCount) 
{
	...
	// 처음 버퍼 생성할때 전달시킬 데이터의 시작주소를 Sub 구조체에 담아서 CreateBuffer 함수에 넣어줍니다.
	tSub = {};
...
void AMesh::Binding() 
{
	// 정점을 프로그래머가 설계하기 때문에, 전달 한 버텍스 버퍼안에서, 하나의 정점의 단위크기를 알려줘야 합니다.
	...
	// 인덱스 숫자 하나의 크기가 몇 바이트인지 픽셀포맷으로 알려줍니다.
...
void AMesh::Render() 
{
	...
	* 그 이전까지는 각 단계별로 실행할 옵션을 설정만 합니다.
	**************************************************************************/
