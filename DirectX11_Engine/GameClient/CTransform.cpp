...
CTransform::CTransform()
	...
	// 스케일을 1로 초기화, 0이면 Render과정에서 시각적으로 안보임
	, m_RelativeScale(Vec3(1.f, 1.f, 1.f))
	, m_Dir{}
	, m_IndependentScale(false)
{
	/***************************************************
	* 디폴트 벡터로 초기화, 오브젝트가 회전을 하지 않은 상태에서
	* 각 방향 벡터의 위치값 입니다.
	* 회전 시, 방향벡터도 같이 회전합니다.
	***************************************************/
...
void CTransform::FinalTick() 
{
	...
	//XMVector3TransformCoord(m_Dir[(UINT)DIR::RIGHT], matRot);	// 동차좌표에 1을 넣어 곱셈
	// 
	// 변환행렬을 적용할 Vec3 벡터를 방향성 데이터로 본다(동차좌표 0로 확장 -> 4행 이동정보 무시)
	//XMVector3TransformNormal(m_Dir[(UINT)DIR::RIGHT], matRot);	// 동차죄표에 0을 넣어 곱셈
	// 
	// 현재는 이동 정보가 없어서 두 코드 중 어느것을 사용해도 상관없다.
