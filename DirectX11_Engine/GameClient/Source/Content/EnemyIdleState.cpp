...
void EnemyIdleState::OnTick()
{
    ...
    //    // Compute a safe movement step and snap when close enough to avoid oscillation
    //    const float snapThreshold = 2.f;
    //    float step = speed * DT;

    //    if (dist <= snapThreshold || step >= dist || dist <= 0.f)
    //    {
    //        // Snap to origin and transition to PATROL
    //        pos = originPos;
    ...
    //    else
    //    {
    //        // Move towards origin without overshooting
    //        int newDir = (dx > 0) ? 1 : -1;
    //        if (newDir != dir)
    //        {
    //            // Ensure scale.x sign matches direction
    //            scale.x = fabsf(scale.x) * (float)newDir;
    //            dir = newDir;
    //        }

    //        float invDist = 1.f / dist;
    //        float move = step; // capped by above check so move < dist
    //        pos.x += dx * invDist * move;
    //        pos.y += dy * invDist * move;
